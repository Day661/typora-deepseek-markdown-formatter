...
插件处理选区时会整段替换内容，无法可靠地把 AI 重排前的高亮、字体颜色等富文本样式重新映射到重排后的文字。因此，如果选区已经包含加粗、高亮、字体颜色、链接、代码、标题、列表或表格，插件会在发送 API 请求前取消本次操作并提示原因。

推荐顺序：先使用 `Ctrl+Alt+L` 轻度清理，再使用 `Ctrl+Alt+M` 结构化排版，最后添加高亮、颜色等手工样式。需要重新处理时，先保留副本或恢复到尚未手工排版的版本。
